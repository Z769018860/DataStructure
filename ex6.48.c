...
void main()
{
  ...
  printf("root->data=%d/np->data=%d/nq->data=%d/n",root->data,p->data,q->data);
  ...
  {
  if(Forf!=NULL)
    printf("结点*p和*q的最近共同祖先是：%d/n",Forf->data);
  else
       printf("结点*p和*q的最近共同祖先是：0/n");
  }//if
  else
 printf("找不到结点*p和*q的最近共同祖先！/n");
